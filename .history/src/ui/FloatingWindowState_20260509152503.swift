...
final class FloatingWindowState: ObservableObject {
  ...
  func updateDrag(petAnchor: CGPoint) {
    ...
    let clampMinX = screenFrame.minX - petAnchor.x
    let clampMaxX = screenFrame.maxX - petAnchor.x
    let clampMinY = screenFrame.minY - petYFromBottom
    let clampMaxY = screenFrame.maxY - petYFromBottom
